...
export function ScanPage() {
  ...
  const progress = running
    ? Math.min(100, Math.round(((step + 1) / scanProgressSteps.length) * 100))
    ...
    <div className="anpu-v6-shell">
      <header className="anpu-v6-page-head">
        <div>
          ...
          <h1 className="anpu-v6-title">Scan with context.</h1>
          <p className="anpu-v6-subtitle">Configure the target on the left. Watch the guardian process the public surface on the right.<span className="cursor-blink ml-1">█</span></p>
        </div>
        <span className={`v6-status ${running ? "warn" : ""}`}>
          <span className="v6-status-dot" /> {running ? "Analyzing" : "Demo ready"}<span className="cursor-blink ml-1">█</span>
        </span>
      </header>
      ...
      <section className="v6-scan-grid">
        <fieldset className="anpu-v6-panel v6-config v7-fieldset">
          ...
          <div className="mt-6">
            <span className="v6-label">Scan profile</span>
            <div className="v6-pills">
              {profiles.map((item) => (
                <button type="button" key={item.value} className={`v6-pill ${profile === item.value ? "active" : ""}`} onClick={() => setProfile(item.value)} disabled={running}>
                  <b>{item.title}</b><small>{item.desc}</small>
                </button>
              ))}
            </div>
          </div>

          <div className="mt-4 grid grid-cols-2 gap-2 border-t border-[#14421E] pt-3 font-mono text-[10px] text-slate-500">
            <div><span>PROFILE</span><b className="mt-1 block text-slate-200">{profile.toUpperCase()}</b></div>
            <div><span>DEPTH</span><b className="mt-1 block text-emerald-400">{profile === "surface" ? "LOW" : profile === "standard" ? "MEDIUM" : "HIGH"}</b></div>
          </div>

          <div className="mt-4 flex items-center justify-between border-t border-[#14421E] pt-3">
        ...
        <section className="anpu-v6-panel v6-live v7-telemetry-panel">
          <div className="v6-live-head">
            <div>
              <div className="anpu-v6-kicker">GUARDIAN TELEMETRY</div>
              <h2 className="mt-1 text-xl font-semibold text-white">𓁹 ANUBIS // NODE 01</h2>
            </div>
            <div className="font-mono text-xs text-slate-500">CRT LINK / ACTIVE</div>
          </div>
          ...
          <div className="v6-terminal-window v7-tomb-window">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-[9px] font-mono text-slate-600">
              <span>ANPU//RADAR</span>
              <span className="text-right">{progress.toString().padStart(3, "0")}% COMPLETE</span>
              <span>PUBLIC SURFACE</span>
              <span className="text-right text-emerald-400">NOMINAL</span>
            </div>

            <div className="mt-5 font-mono text-[10px] leading-6 text-slate-500">
              {scanProgressSteps.map((item, i) => (
                <div key={item} className={i <= step ? "text-slate-200" : ""}>
                  <span className={i < step || (!running && progress === 100) ? "text-emerald-400" : i === step ? "text-amber-300" : "text-slate-700"}>
                    {i < step || (!running && progress === 100) ? "✓" : i === step ? "›" : "·"}
                  </span>
          ...
          <div className="v6-telemetry">
            <div><small>ENDPOINTS</small><b>{running ? Math.round(progress * 1.7) : progress === 100 ? 168 : 0}</b></div>
            <div><small>REQUEST RATE</small><b>{running ? 18 : 0}<span className="ml-1 text-xs text-slate-500">/s</span></b></div>
            <div><small>ELAPSED</small><b>{elapsed}s</b></div>
          </div>
          ...
          <div className="mt-4 grid grid-cols-3 gap-2 text-[10px] font-mono text-slate-500">
            <div className="border border-white/5 p-2"><Network className="mb-1 h-3.5 w-3.5 text-emerald-400" />PUBLIC</div>
            <div className="border border-white/5 p-2"><Gauge className="mb-1 h-3.5 w-3.5 text-emerald-400" />SIGNALS</div>
            <div className="border border-white/5 p-2"><Clock3 className="mb-1 h-3.5 w-3.5 text-emerald-400" />REALTIME</div>
          </div>
        </section>
      </section>
    </div>
